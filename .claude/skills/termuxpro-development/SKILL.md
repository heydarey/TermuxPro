...
发布不是停止发布。长期维护期间每周至少做一次稳定版发布窗口评审，原则上每 7 天至少形成一次稳定版
`RELEASED` 结果；满足稳定和价值条件时必须推进候选/正式发布，未满足时记录 `HOLD` 证据和下次复评
时间，本周已发布时记录最新稳定版和不重复发版依据。最近稳定版接近或超过 7 天时，应优先复核发布条件，
不能长期只迭代不发包。
