...
发布不是停止发布。长期维护期间每周至少做一次稳定版发布窗口评审；稳定版发布 SLA 为最近稳定 Release
后 7 天内必须给出 `RELEASED` 或有证据的 `HOLD`。满足稳定和价值条件时必须推进候选/正式发布，
未满足时必须在 backlog 或发布评审报告中记录 `HOLD` 原因、责任角色、解除条件和下次复评时间；本周
已发布时记录最新稳定版、下次最晚复评日期和不重复发版依据。最近稳定版接近或超过 7 天且没有落盘
`HOLD` 时，应优先复核发布条件或启动发布列车，不能长期只迭代不发包。每轮日常迭代开头都要检查
最新稳定 Release 年龄：满 5 天进入发布预警，满 7 天没有 `RELEASED/HOLD` 结论时禁止继续普通切片。
