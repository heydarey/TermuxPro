...
`0.10.1` 是 ARM64 正式版，面向 Android 7 及以上设备，优先验证 Android 14–16。当前阶段聚焦：
...
远端文件写入、Mosh、增强远端代理以及应用内模型调用尚未作为 `0.10.1` 稳定能力承诺。
...
克制发布不是停止发布；长期维护期间每个自然周至少进行一次稳定版发布窗口评审，原则上每 7 天至少形成
一次稳定版 `RELEASED` 结果，评审结论可以是 `READY`、`HOLD` 或 `RELEASED`。没有达到质量或价值标准时
记录 HOLD 原因、解除条件和下一次复评时间，不为了显示进度制造候选包；本周已经发布稳定版时，记录
最新稳定版、下次最晚复评日期和不重复发版依据。反过来，如果最近几天没有新增 P0/P1、`dev` 门禁和
模拟器/发布回归稳定，且已累计真实用户可感知价值，就应推进候选/正式发布，不能长期只迭代不发稳定版。
若最近一个稳定 Release 已接近或超过 7 天，负责人应优先执行发布评审，而不是继续开启新的 P2/P3 日常切片。每个发布
列车默认只有一个 `rc.1`，仅 P0/P1、门禁失败或产物不可安装才递增候选编号。发布范围冻结后，新 P2 需求
进入下一版本；候选版全部门禁通过且没有 P0/P1 后，在同一维护周期合入 `master` 并创建稳定 Release。
