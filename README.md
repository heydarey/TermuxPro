...
克制发布不是停止发布；长期维护期间每个自然周至少进行一次稳定版发布窗口评审，评审结论可以是
`READY`、`HOLD` 或 `RELEASED`。没有达到质量或价值标准时记录 HOLD 原因、解除条件和下一次复评时间，
不为了显示进度制造候选包。反过来，如果最近几天没有新增 P0/P1、`dev` 门禁和模拟器/发布回归稳定，
且已累计真实用户可感知价值，就应推进候选/正式发布，不能长期只迭代不发稳定版。若最近一个稳定
Release 已接近或超过 7 天，负责人应优先执行发布评审，而不是继续开启新的 P2/P3 日常切片。每个发布
